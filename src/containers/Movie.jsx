...
const usegetMovie = (filmID) => {
  return useQuery({
    queryKey: ['movie', filmID],
    queryFn: async () => {
      const { data } = await axios.get(
        `https://api.themoviedb.org/3/movie/${filmID}?api_key=331e14e782094dc12557a69cadeee6b6&`
      );
      return data;
    },
...
export const Movie = () => {
  ...
  const { data: movieData, isLoading: movieLoading } = usegetMovie(filmID)
  return (
    <>
      {filmID == undefined ? (
        ...
        movieLoading ? (
          ...
          <div className='mt-[50px] flex flex-col justify-start w-full  h-[600px] bg-gray-200 border shadow-xl p-4 rounded-xl'>
            <div className='flex flex-row w-full gap-x-10 my-auto'>
              <img src={`http://image.tmdb.org/t/p/w500/${movieData.poster_path}`} alt={`${movieData.original_title}`} className="w-[40%] h-[500px]" />
              <div className='flex flex-col gap-y-10 bg-slate-700 w-full p-3'>
                <h1 className='font-mono text-3xl text-white font-bold tracking-[10px] text-center'>{movieData.original_title}</h1>
                <div className='flex flex-row gap-5 text-white font-light justify-center text-xs'>
                  {movieData.genres.map((value, index)=> {
                    return (
                ...
                <p className='text-bs text-gray-200 bg-slate-900 p-4 text-justify rounded-xl'>{movieData.overview}</p>
